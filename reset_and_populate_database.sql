...
-- This script will clean up the database and properly populate it with card definitions
-- Run this in Supabase SQL Editor

-- Step 1: Clean up existing data (in correct order due to foreign key constraints)
DELETE FROM public.user_cards;
DELETE FROM public.card_definitions;
DELETE FROM public.pack_slots;
DELETE FROM public.pack_configs;
DELETE FROM public.card_sets WHERE code = 'BASE';

-- Step 2: Re-insert the base set
...
-- Step 3: Re-insert pack configuration
DO $$
DECLARE
  base_set_id uuid;
  cfg_id uuid;
...
-- Step 4: Insert card definitions
DO $$
DECLARE
  base_set_id uuid;
...
BEGIN
  ...
  RAISE NOTICE 'Database reset and populated successfully!';
END $$;

-- Step 5: Verify the setup
SELECT 
  'Card Sets' as table_name, COUNT(*) as count FROM public.card_sets
UNION ALL
SELECT 
  'Card Definitions' as table_name, COUNT(*) as count FROM public.card_definitions
UNION ALL
SELECT 
  'Pack Configs' as table_name, COUNT(*) as count FROM public.pack_configs
UNION ALL
SELECT 
  'Pack Slots' as table_name, COUNT(*) as count FROM public.pack_slots
UNION ALL
SELECT 
  'User Cards' as table_name, COUNT(*) as count FROM public.user_cards;
